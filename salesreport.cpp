...
//    GROUP BY Day
        ...
        qry2.prepare("SELECT Day, COUNT(DISTINCT MemberName) as Totalcustomers, COUNT(DISTINCT CASE WHEN MembershipType = 'Executive' THEN MemberName END) as Executive_Members, COUNT(dISTINCT CASE WHEN MembershipType = 'Regular' THEN MemberName END) as Regular_Members, ROUND(SUM(PriceOfProduct * Quantity * (1 + .0775)),2) as TotalRevenue FROM items INNER JOIN Members ON CustomerPurchasedID = MemberNumber WHERE Day = '"+search+"' GROUP BY Day");
        if (qry2.exec())
        {
            modal2->setQuery(std::move(qry2));
